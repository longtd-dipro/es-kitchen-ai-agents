...
>
> **Cập nhật 2026-09-22 — user đã chốt 4 mục:** OQ-03 · OQ-09 · OQ-10 · OQ-16 (xem cột Ảnh hưởng, đã gạch ngang). 4 quyết định này đã được dùng làm cơ sở sinh test case, **không** còn tag `[UNCONFIRMED]`. 12 mục còn lại vẫn mở.

| ID | Câu hỏi | Nguồn trong Figma | Ảnh hưởng nếu chưa chốt |
|---|---|---|---|
| ~~OQ-03~~ ✅ | ~~Nút `CSV出力` ở `AW_AGCY_001` là **xuất** hay **nhập** CSV?~~ | `30079:156426` H.8 | **ĐÃ CHỐT 2026-09-22 (user):** là **XUẤT CSV** — tải xuống danh sách theo điều kiện lọc hiện tại, đồng nhất với `AW_REFE_001` và `AW_FEPA_001`. Cột Chi tiết trong Figma mô tả sai, cần BA/Designer sửa. |
...
| ~~OQ-10~~ ✅ | ~~Cập nhật trạng thái sau khi kỳ hạn thanh toán mãn hạn?~~ | Text node `29419:271727` | **ĐÃ CHỐT 2026-09-22 (user):** khi kỳ hạn thanh toán mãn hạn → `ステータス` **tự chuyển** `契約中` → `満了`, batch **ngừng sinh** `支払履歴` cho hồ sơ đó từ tháng kế tiếp. Cột `有効紹介数` vẫn chưa có trong màn nào → giữ là hạng mục chờ Designer. |
...
| ~~OQ-09~~ ✅ | ~~`算定合計額` đã gồm thuế hay chưa?~~ | `30148:165790` L.8 | **ĐÃ CHỐT 2026-09-22 (user):** **đã gồm thuế (税込)** — nhất quán với `フィー金額（円）税込` ở `AW_FEPL_002` (Figma ghi rõ không có field thuế suất / số tiền chưa thuế). |
...
| ~~OQ-16~~ ✅ | ~~Nút bị chặn quyền thì ẩn hay disable?~~ | `30033:237981` H.1 | **ĐÃ CHỐT 2026-09-22 (user):** **ẩn hoàn toàn**. Không có quyền → nút không render; truy cập thẳng URL màn đăng ký/sửa cũng phải bị chặn. |
