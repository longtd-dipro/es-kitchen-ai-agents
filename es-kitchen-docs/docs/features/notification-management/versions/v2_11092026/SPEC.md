...
> Entry point cho TL Design / Designer / QC — đọc section này đầu tiên. Đây là single source of truth cho mọi downstream agent.

| # | Output | Status | Path / URL | Note |
|---|---|---|---|---|
| 0 | **SPEC.md** | ✅ Done | `es-kitchen-docs/docs/features/notification-management/SPEC.md` | File này |
| 1 | **Figma Frame — Flow Tổng Quan** | ✅ Done | [Mở Figma](https://www.figma.com/design/VKAAOyoSPvgoB3H2qdeeV3/ES-Kitchen-phase-2?node-id=31113-48836) | Business Logic Flow (2 flows) + Technology Table (9 tech) + Sitemap WBS Tree. Page: `Project_Demo_BA_AGENT` |
| 2 | **Figma Frame — Screen Flow** | ❌ Skipped — user chỉ yêu cầu Output 0 + 1 | — | Giao diện screens đã có sẵn tại Figma input (node 16942-51758). Nếu cần: Designer tạo từ `## Screens` + `## Screen Details` |
| 3 | **Figma Frame — Screens + Items** | ❌ Skipped — user chỉ yêu cầu Output 0 + 1 | — | Figma design đã có tại node-id=16942-51758. Nếu cần: Designer tạo high-fi từ SPEC |
| 4 | **HTML Prototype** | ❌ Skipped — user chỉ yêu cầu Output 0 + 1 | — | Nếu cần prototype: chạy BA Agent lại với request "Tạo HTML prototype cho notification-management" |

**Figma outputs — BA Agent:**
- **Output 1 — Flow Tổng Quan:** https://www.figma.com/design/VKAAOyoSPvgoB3H2qdeeV3/ES-Kitchen-phase-2?node-id=31113-48836
  - Page: `Project_Demo_BA_AGENT`
  - Nội dung: Flow 1 (Cấu hình Notification Types) + Flow 2 (Quản lý Email Notification) + Technology Table + Sitemap WBS Tree

**Figma Design Source (input — giao diện có sẵn):**
...
- **Tech Lead Design:** Đọc Output 1 Figma (Flow + Tech Stack) + `## Screens` + `## Screen Details` + `## Acceptance Criteria` → tạo `Design-Technical.md` cho `es-kitchen-api` và `es-kitchen-web-admin`.
- **Designer:** Đọc `## Screens` + `## Screen Details` + Figma input (giao diện có sẵn) → tạo high-fidelity screens → điền cột `Figma Link` trong `## Screens`.
- **QC:** Đọc `## Acceptance Criteria` + `## Alternative Flows & Edge Cases` + Output 1 Flow (để hiểu business logic) → sinh test cases.
